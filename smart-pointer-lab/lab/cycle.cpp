...
int main() {
    ...
    //actually, a.use_count=1 b.use_count=1, with the weak_ptr fix
...
struct Order {
    int id_;
    std::string symbol_;
    //order should hold a non-owning reference to the trader, to avoid circular shared references
...
//inheriting from enable_shared_from_this to create shared_ptrs of this, with the same control block
struct Trader : public std::enable_shared_from_this<Trader> {
    ...
    void place_order(std::shared_ptr<Order> o) {
        active_order_ = o;
        //problem is how to set o's owner
        //not correct since it will create a new control block managing this, for every order placed(not the same control block for all)
        //o->owner_ = std::shared_ptr(this);

        //correct way using shared_from_this
        //o->owner_ = shared_from_this();

        //even better way using weak_from_this
        o->owner_ = weak_from_this();
